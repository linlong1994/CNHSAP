...
def _reference_check(software,reference_path,warningString):
	if reference_path == None or (not os.path.exists(reference_path)):
		...
		if software == 'samtools':
			...
			if not glob.glob(samtools_index):
		...
		if software == 'bwa':
			...
			bwa_index_exists = glob.glob(bwa_index1) and glob.glob(bwa_index2) and glob.glob(bwa_index3) and glob.glob(bwa_index4) and glob.glob(bwa_index5)
			if bwa_index_exists:
				return True
			else:
...
def _align(fq,sample_id,outdir,faidxed_reference,indexed_reference,bwa,samtools,bgzip,tabix,classification=False,stats=False,cpu=1):
	if len(fq.split(','))==1:	
		...
		fq = fq.replace(',',' ')
		sys.stderr.write(" %s mem -t %s %s %s| %s view -h -Sb - > %s/%s.bam\n" % (bwa,cpu,indexed_reference,fq,samtools,outdir,sample_id))
		os.system(" %s mem -t %s %s %s| %s view -h -Sb - > %s/%s.bam\n" % (bwa,cpu,indexed_reference,fq,samtools,outdir,sample_id))
	else:
		sys.stderr.write("[ERROR] input fq is not correct: %s " % (fq))
		sys.exit(1)
...
def _single_reference_abundance(reads_classification_file,bamstats_summury,reference_fai,species_name,samtools,bam):
	...
	sys.stderr.write('perl %s %s %s %s %s %s %s > %s\n' % (getAbunceScript,bamstats_summury,reads_classification_file,reference_fai,species_name,samtools,bam,abundanceFile))
	os.system('perl %s %s %s %s %s %s %s > %s' % (getAbunceScript,bamstats_summury,reads_classification_file,reference_fai,species_name,samtools,bam,abundanceFile))
	return abundanceFile

def _align_multiReference(fq,sample_id,outdir,species_list,reference_list,bamstats_summury,bwa,samtools,bgzip,tabix):
	...
	for i in range(len(reference_list)):
		...
		reference_fai = glob.glob(reference_list[i] + '*.fai')[0]
		...
		if faidxed_reference_precondition and bwa_reference_precondition:
			...
			abundanceFile = _single_reference_abundance(reads_classification_file,bamstats_summury,reference_fai,species_name,samtools,sortBam)
	return
...
def _metaphlan2run(metaphlan2,fq,sample_id,bowtie2db,mpa_pkl,outdir,nproc=1,stat='avg_g',t='rel_ab_w_read_stats',input_type='fastq'):
	...
	if metaphlan2_precondition and bowtie2db_precondition and mpa_pkl_precondition:
...
def _kraken2(kraken2,fq,sample_id,kraken2db,outdir,thread=1,):
	...
	if len(fq.split(',')) == 1:
		...
		fq = fq.replace(',',' ')
		os.system('time %s --db %s %s --quick --use-names --gzip-compressed --paired --threads %s --use-mpa-style --report %s --output %s ' % (kraken2,kraken2db,fq,thread,kraken2report,kraken2output) )
	else:
		sys.stderr.write("[ERROR] Kraken2 input fq is not correct: %s " % (fq))
		sys.exit(1)
...
def _kraken2run(kraken2,fq,sample_id,kraken2db,outdir,thread=1):
	...
	if kraken2_preconditon and kraken2db_precondition :
...
class GnhaRunner(object):
	...
	def gnha_runner(self):
		...
		for i in range(int(self.nCPU)):
			...
			#_align_multiReference(fq,sample_id,outdir,species_list,reference_list,bamstats_summury,bwa,samtools,bgzip,tabix)
			processes.append(Process(target = _align_multiReference,args=(
											self.fq,
											self.sample_id,
											sub_dir,
